...
public interface IPatientDao extends JpaRepository<Patient, Integer> {
	...
	int insertIntoPatients(@Param("id") int id, @Param("user_id") int user_id, @Param("doctor_id") int doctor_id,
			@Param("ward_id") int ward_id, @Param("date_of_admission") Date date_of_admission,
			@Param("blood_group") String blood_group, @Param("dob") Date dob,
			@Param("prescription") String prescription, @Param("bed_alloted") int bed_alloted,
			@Param("payment_status") String payment_status, @Param("patient_problem") String patient_problem);
